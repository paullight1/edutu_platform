...
| `SUPABASE_EMAIL_API_KEY` | Server-only email provider key. Missing configuration skips delivery rather than disclosing it. |
...
`edutumobile/supabase/migrations/033_weekly_digest_job_lock.sql` before enabling
the schedule. It creates `public.weekly_digest_jobs` with:
...
- `public.claim_weekly_digest_job(integer, date)`, an atomic
  `INSERT ... ON CONFLICT DO NOTHING` claim RPC available only to `service_role`.

The function claims the job before reading preferences, profiles, goals, or
bookmarks and before calling the email provider. A duplicate claim returns
`{ "sent": 0, "skipped": 0 }` and performs no recipient or email work. The
claim is durable to prevent retry-driven duplicate sends; any recovery of a
partially delivered job requires an operator-reviewed database change.
