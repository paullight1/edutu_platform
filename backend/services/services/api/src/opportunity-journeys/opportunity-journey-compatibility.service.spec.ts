...
describe("OpportunityJourneyCompatibilityService", () => {
  ...
  it("lets an application override a bookmark for the same opportunity", async () => {
    ...
    const store = legacyStore({
      listBookmarks: jest
        .fn()
        .mockResolvedValue([{ id: "bookmark-1", opportunityId }]),
  ...
  it("keeps an existing journey authoritative over older legacy records", async () => {
    const current = {
      id: "journey-1",
      opportunityId: "11111111-1111-4111-8111-111111111111",
      state: "shortlisted",
...
it("reconciles a newer forward update from the existing Applications screen", async () => {
  const current = {
    id: "journey-1",
  ...
  expect(repo.updateJourneyVersioned).toHaveBeenCalledWith(
    expect.objectContaining({
      expectedVersion: 2,
      patch: expect.objectContaining({ state: "interview" }),
    }),
  );
});
