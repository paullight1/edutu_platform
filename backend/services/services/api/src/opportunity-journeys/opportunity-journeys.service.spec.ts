...
function createHarness(
  ...
  const repository: Record<string, jest.Mock> = {
    findMutationReplay: jest.fn().mockResolvedValue(null),
    ...
    listTasksForJourneys: jest.fn(async () => state.tasks),
...
describe("My Plan lifecycle safeguards", () => {
  it("returns a ready journey to preparation when a required task is reopened", async () => {
    const { service, state } = createHarness({
      existing: journey({ state: "ready_to_apply" }),
    });
    ...
    const result = await service.updateTask(
      ...
      state.tasks[0].id,
      {
        status: "pending",
        expectedVersion: 1,
        idempotencyKey: "reopen-required",
      },
    );
    expect(result.journey.state).toBe("preparing");
  });
  it("does not let task changes rewind a submitted application", async () => {
    const { service, state, repository } = createHarness({
      existing: journey({ state: "applied" }),
    });
    state.tasks = [task({ status: "completed" })];
    await expect(
      service.updateTask(USER_ID, state.current!.id, state.tasks[0].id, {
        status: "pending",
        expectedVersion: 1,
        idempotencyKey: "rewind-submitted",
      }),
    ).rejects.toMatchObject({ code: "INVALID_JOURNEY_TRANSITION" });
    expect(repository.updateTaskStatus).not.toHaveBeenCalled();
  });
  it("accepts explicit confirmation of an application submitted outside the checklist", async () => {
    const { service, state } = createHarness({
      existing: journey({ state: "shortlisted" }),
    });
    const result = await service.confirmApplication(
      USER_ID,
      state.current!.id,
      { expectedVersion: 1, idempotencyKey: "external-confirm" },
    );
    expect(result.journey.state).toBe("applied");
    expect(result.journey.appliedAt).toBeInstanceOf(Date);
  });
  it("does not activate a closed outcome again when the opportunity CTA is pressed", async () => {
    const { service, repository } = createHarness({
      existing: journey({ state: "offer" }),
    });
    const result = await service.createJourney(USER_ID, {
      opportunityId: OPPORTUNITY_ID,
      action: "pursue",
      idempotencyKey: "repeat-after-offer",
    });
    expect(result.journey.state).toBe("offer");
    expect(repository.updateJourneyVersioned).not.toHaveBeenCalled();
  });
  it("rejects an intent belonging to a different account", async () => {
    const { service } = createHarness();
    await expect(
      service.createJourney(USER_ID, {
        opportunityId: OPPORTUNITY_ID,
