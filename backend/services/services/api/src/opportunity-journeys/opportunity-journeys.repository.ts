...
export interface JourneyWithDeadline extends OpportunityJourney {
  ...
  opportunitySummary?: { id: string; title: string; deadline: Date | null };
}
...
export class OpportunityJourneysRepository {
  ...
  async listJourneysByStage(
    ...
    return rows.map((row: any) => ({
      ...
      opportunitySummary: {
        ...
        deadline: row.opportunityDeadline ?? null,
      },
    }));
  }
  ...
  async listTasksForJourneys(
    userId: string,
    journeyIds: string[],
  ): Promise<OpportunityJourneyTask[]> {
    if (!journeyIds.length) return [];
    const rows = await this.database
      .select({ task: opportunityJourneyTasks })
      .from(opportunityJourneyTasks)
      .innerJoin(
        userOpportunityJourneys,
        eq(userOpportunityJourneys.id, opportunityJourneyTasks.journeyId),
      )
      .where(
        and(
          eq(userOpportunityJourneys.userId, databaseUserId(userId)),
          inArray(userOpportunityJourneys.id, journeyIds),
        ),
      )
      .orderBy(asc(opportunityJourneyTasks.position))
      .execute();
    return rows.map((row: { task: OpportunityJourneyTask }) => row.task);
  }
