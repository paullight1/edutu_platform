...
export class OpportunityJourneyOperationsRepository extends OpportunityJourneysRepository {
  ...
  async findMutationReplay(
    userId: string,
    journeyId: string,
    idempotencyKey: string,
    requestHash: string,
  ): Promise<OpportunityJourney | null> {
    const [event] = await this.operationsDatabase
      .select()
      .from(opportunityJourneyEvents)
      .where(
        and(
          eq(opportunityJourneyEvents.userId, convertedUserId(userId)),
    ...
    if (
      ...
      event.metadata?.requestHash !== requestHash
    ) {
      throw new OpportunityJourneyRepositoryError(
    ...
    return this.findJourneyForUser(userId, journeyId);
  }
