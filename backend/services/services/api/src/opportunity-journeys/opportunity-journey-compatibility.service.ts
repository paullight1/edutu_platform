import { isClosedJourneyState } from "./opportunity-journey-state";
...
interface LegacyJourneyCandidate {
  ...
  appliedAt: Date | null;
  closedAt: Date | null;
  outcome: OpportunityJourney["outcome"];
  updatedAt: Date | null;
}
...
function applicationCandidate(
  ...
  const state =
    application.metadata?.source === "mobile_detail" &&
    application.status === "submitted"
      ? "application_opened"
      : APPLICATION_STATE[application.status.toLowerCase()];
  if (!state) return null;
  const appliedAt =
    state === "applied" || state === "interview" || STATE_STRENGTH[state] >= 8
      ? dateValue(application.submittedAt)
      : null;
  const closedAt =
    STATE_STRENGTH[state] >= 8
  ...
  return {
    ...
    appliedAt,
    closedAt,
    outcome: isClosedJourneyState(state) ? state : null,
...
function legacyApplicationStatus(
  state: OpportunityJourneyState,
): string | null {
  return (
    {
...
export class OpportunityJourneyCompatibilityService {
  constructor(
    ...
    private readonly legacyStore: OpportunityJourneyLegacyStore = new DatabaseOpportunityJourneyLegacyStore(),
  ) {}
  ...
  async reconcileUser(userId: string): Promise<{
    ...
    let updated = 0;
    let skipped = 0;
    const unsupported = applications.filter(
      (application) => !APPLICATION_STATE[application.status.toLowerCase()],
    ).length;

    const existing = new Map(
      (await this.repository.listJourneysForUser(userId)).map((item) => [
        item.opportunityId,
        item,
      ]),
    );
    for (const candidate of candidates) {
      const current = existing.get(candidate.opportunityId);
      ...
      // Accept a newer forward application update from the existing Applications
      // screen/web client. Never rewind a plan or overwrite a decided outcome.
      if (
        candidate.source === "application" &&
        candidate.updatedAt &&
        current.updatedAt &&
        candidate.updatedAt.getTime() > current.updatedAt.getTime() &&
        STATE_STRENGTH[current.state] < 8 &&
        STATE_STRENGTH[candidate.state] > STATE_STRENGTH[current.state] &&
        STATE_STRENGTH[candidate.state] >= STATE_STRENGTH.applied
      ) {
        await this.repository.updateJourneyVersioned({
          ...
          idempotencyKey: `${idempotencyKey}:${candidate.state}:${candidate.updatedAt.toISOString()}`,
          eventType: "legacy_application_updated",
          source: "migration",
          metadata: { legacySourceId: candidate.sourceId },
        });
        updated += 1;
      } else {
        skipped += 1;
