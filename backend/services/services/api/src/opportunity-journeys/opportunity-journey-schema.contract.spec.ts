...
// Reuse one PostgreSQL WASM engine, resetting its schema between tests. This
// avoids repeatedly allocating an engine while retaining database isolation.
let sharedDatabase: TestDatabase;
beforeAll(async () => {
  sharedDatabase = await createOpportunityJourneyTestDatabase();
}, 60000);
afterAll(async () => {
...
async function withMigratedDatabase(
  assertion: (database: TestDatabase) => Promise<void>,
): Promise<void> {
  await sharedDatabase.exec(`
    drop schema public cascade;
    create schema public;
    create table public.opportunities (id uuid primary key default gen_random_uuid(), title text not null default 'Opportunity');
  `);
  await applyOpportunityJourneyMigration(sharedDatabase);
  await assertion(sharedDatabase);
}
...
async function seedOpportunityAndIntent(database: TestDatabase): Promise<void> {
  await database.exec(`
...
async function seedJourney(database: TestDatabase): Promise<void> {
  await database.exec(`
...
describe("opportunity journey Drizzle mappings", () => {
  it("maps the four API-owned tables and canonical camelCase columns", () => {
...
describe("opportunity journey migration", () => {
  it("creates all tables, indexes, RLS boundaries, and applies twice", async () => {
  ...
  it("enforces active-intent and active-primary uniqueness", async () => {
    await withMigratedDatabase(async (database) => {
      ...
      await seedJourney(database);
      await expectSqlFailure(
        database,
        `insert into public.user_opportunity_journeys
           (user_id, opportunity_id, state, priority)
         values ('${USER_ONE}', '${OPPORTUNITY_ONE}', 'shortlisted', 'none');`,
      );
      await expectSqlFailure(
        database,
        `insert into public.user_opportunity_journeys
           (user_id, opportunity_id, state, priority)
         values ('${USER_ONE}', '${OPPORTUNITY_TWO}', 'pursuing', 'primary');`,
      );
      await database.exec(`
        insert into public.user_opportunity_journeys
          (user_id, opportunity_id, state, priority)
        values ('${USER_TWO}', '${OPPORTUNITY_TWO}', 'pursuing', 'primary');
      `);
    });
  }, 60_000);

  it("rejects values outside every bounded vocabulary", async () => {
    await withMigratedDatabase(async (database) => {
      await seedOpportunityAndIntent(database);
      ...
      await database.exec(`
        insert into public.opportunity_journey_tasks
          (journey_id, task_type, title, position)
        values ('${JOURNEY_ONE}', 'eligibility', 'Confirm eligibility', 0);

        insert into public.opportunity_journey_events
          (user_id, journey_id, opportunity_id, event_type, source, idempotency_key)
        values (
          '${USER_ONE}',
          '${JOURNEY_ONE}',
          '${OPPORTUNITY_ONE}',
      ...
      await expectSqlFailure(
        ...
        `update public.opportunity_journey_events
         set source = 'unknown'
         where idempotency_key = 'activate-1';`,
      );
    });
  }, 60_000);

  it("enforces task ordering and event idempotency", async () => {
    await withMigratedDatabase(async (database) => {
      await seedOpportunityAndIntent(database);
      await seedJourney(database);
      await database.exec(`
      ...
      await expectSqlFailure(
        ...
        `insert into public.opportunity_journey_tasks
           (journey_id, task_type, title, position)
         values ('${JOURNEY_ONE}', 'document', 'Collect transcript', 0);`,
      );
      await expectSqlFailure(
        database,
        `insert into public.opportunity_journey_events
