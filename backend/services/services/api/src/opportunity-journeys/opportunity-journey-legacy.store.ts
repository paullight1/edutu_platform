...
import { db } from "../db";
import { matchUserIdRef, toDatabaseUserId } from "../common/user-id";
...
export class DatabaseOpportunityJourneyLegacyStore implements OpportunityJourneyLegacyStore {
  ...
  async listBookmarks(userId: string): Promise<LegacyOpportunityBookmark[]> {
    const result = await this.database.execute(sql`
      ...
      where ${matchUserIdRef("user_id", toDatabaseUserId(userId))}
  ...
  async ensureApplication(
    ...
    await this.database.execute(sql`
      ...
      set status = excluded.status,
          submitted_at = coalesce(public.opportunity_applications.submitted_at, excluded.submitted_at),
          notes = coalesce(public.opportunity_applications.notes, excluded.notes),
          metadata = coalesce(public.opportunity_applications.metadata, '{}'::jsonb) || excluded.metadata,
          updated_at = now()
    `);
  }
