...
export class OpportunitiesService {
  ...
  /**
   * Catalog data-quality scorecard for the admin Engine dashboard: the health
   * numbers an operator would otherwise only learn from a manual audit. Raw
   * counts (+ the `active` denominator) so the UI renders rates without the
   * backend baking in thresholds.
   */
