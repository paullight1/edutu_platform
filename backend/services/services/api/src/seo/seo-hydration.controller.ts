...
import { findSeoCategory, type SeoCategory } from "./seo-data";
import {
  escapeHtml,
  injectSeoIntoShell,
  renderSeoDocument,
...
export class SeoHydrationController {
  ...
  private prepareResponse(res: Response, status: number, robots: string): void {
    res.status(status);
  ...
  async opportunityCategory(
    ...
    if (!category) {
      return this.respond(
        res,
        this.notFoundPage("Opportunity category", this.absolute(requestedPath)),
        404,
      );
    }
    ...
    const baseTitle = category.title.replace(/\s*\|\s*Edutu\s*$/i, "").trim();
    const title = `${baseTitle}${page > 1 ? ` — Page ${page}` : ""} | Edutu`;
