...
-- Operational audit query for any balance that would violate the API credit
-- contract. The view is intentionally empty for healthy accounts and is
-- readable only by the service role for scheduled reconciliation checks.
create or replace view public.api_credit_balance_integrity_audit as
select user_id::text as user_id, credits
from public.profiles
where credits is null or credits < 0;

revoke all on public.api_credit_balance_integrity_audit from public, anon, authenticated;
