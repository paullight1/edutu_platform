...
const mockGetStoredPushToken = jest
  .fn()
  .mockResolvedValue("ExponentPushToken[test]");
...
const menuGroups = Array.from({ length: 6 }, (_, index) => ({
  group: {
    id: `group-${index + 1}`,
    slug: `group-${index + 1}`,
    name: `Community Group ${index + 1}`,
...
let mockBookmarkOpportunities: Array<{
  title: string;
...
function makeBuilder(table: string) {
  ...
  const builder: any = {
    ...
    then: (resolve: any, reject: any) => {
      ...
      Promise.resolve(resolveQuery(table, state)).catch(reject),
...
jest.mock("../components/ui/AdBanner", () => ({
  BANNER_PRESETS: { buildCV: { id: "build-cv" } },
  AdBanner: ({ onPress }: { onPress?: () => void }) => {
    const React = require("react");
    const { Text, TouchableOpacity } = require("react-native");
    return (
      <TouchableOpacity onPress={onPress}>
        <Text>Build Your Perfect CV</Text>
      </TouchableOpacity>
    );
  },
}));
...
jest.mock(
  "@edutu/core/src/services/communities",
  () => ({
...
jest.mock(
  "@edutu/core/src/hooks/useOpportunities",
  () => ({
    useOpportunities: () => ({
...
jest.mock("../lib/widgetSuiteSync", () => ({
  clearWidgetSuiteData: jest.fn().mockResolvedValue(undefined),
}));
...
describe("mobile profile routes", () => {
  beforeEach(() => {
    ...
    mockFetchGroups.mockResolvedValue(menuGroups);
  ...
  it("routes from the profile overview into primary destinations and shows stats", async () => {
    ...
    expect(queryByText("Community groups")).toBeNull();
    expect(queryByText("Chats")).toBeNull();
    expect(queryByText("Build Your Perfect CV")).toBeNull();
    expect(queryByText("Revisit shortlisted opportunities")).toBeNull();

    pressNearestTouchTarget(getByText("Applied"));
    expect(mockPush).toHaveBeenCalledWith("/applied");

    pressNearestTouchTarget(getByText("Documents"));
    expect(mockPush).toHaveBeenCalledWith("/profile/documents");

    pressNearestTouchTarget(getByText("Roadmaps"));
  ...
  it("loads profile data and saves updates from the edit screen", async () => {
    ...
    fireEvent.changeText(
      getByPlaceholderText("Your full name"),
      "Ada Lovelace",
    );
  ...
  it("encodes quiet hours off as a zero-length window the backend understands", async () => {
    ...
    await waitFor(() =>
      expect(getAllByRole("switch")[3].props.value).toBe(true),
    );
