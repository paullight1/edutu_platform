...
describe('mobile opportunity detail route', () => {
  ...
  it('places optional application support after source-backed opportunity details', async () => {
    mockOpportunity = makeOpportunity();
    mockGetOpportunity.mockResolvedValue(mockOpportunity);

    const { getByText, toJSON } = render(<OpportunityDetailScreen />);
    await waitFor(() => expect(getByText('Global Fellowship')).toBeTruthy());

    const renderedText = collectRenderedText(toJSON());
    const about = renderedText.indexOf('About This Opportunity');
    const support = renderedText.indexOf('Help me apply');

    expect(about).toBeGreaterThan(-1);
    expect(support).toBeGreaterThan(about);
  });
