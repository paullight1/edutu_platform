...
jest.mock('@edutu/core/src/services/communities', () => {
  ...
  return {
    ...
    fetchGroupResources: (...args: unknown[]) => mockFetchGroupResources(...args),
    resolveCommunityAttachmentUrl: (...args: unknown[]) => mockResolveAttachmentUrl(...args),
...
beforeEach(() => {
  ...
  mockFetchGroupResources.mockResolvedValue({ resources: [], nextCursor: null });
...
describe('linked opportunity on a group profile', () => {
  ...
  it('lists persisted resources and resolves private access only when opened', async () => {
    mockFetchGroupResources
      ...
      nextCursor: { before: '2026-08-05T10:00:00.000Z', beforeId: 'resource-1' },
      resources: [
        {
          id: 'resource-1',
          groupId: 'group-1',
          ...
          sender: { userId: 'owner-1', displayName: 'Amina Yusuf', avatarUrl: null },
        ...
        resources: [
          {
            ...
            sender: { userId: 'member-2', displayName: 'Tobi Ade', avatarUrl: null },
            createdAt: '2026-08-01T10:00:00.000Z',
          },
        ],
      });
    ...
    await waitFor(() => expect(getByText('application-checklist.pdf')).toBeTruthy());
    ...
    expect(open).toHaveBeenCalledWith('https://storage.example.test/private-resource');
