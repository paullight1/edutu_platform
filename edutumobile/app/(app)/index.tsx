...
} from "@edutu/core/src/types/opportunity";
import { toSafeUUID } from "@edutu/core/src/utils/auth";
import {
  recordOpportunitySignal,
  type DismissReason,
} from "@edutu/core/src/services/opportunitySignals";
...
import {
  DISCOVERY_TILE_GLYPHS,
  DISCOVERY_TILE_GRADIENTS,
} from "../../lib/discoveryTileGlyphs";
...
const EDITOR_TILE_SIZE_STEPS: Array<{
  ...
  if (size === "long") {
    const row = (
      <View style={styles.longTileRow}>
        ...
        <Text style={styles.longTileTitle} numberOfLines={1}>
          {title}
        </Text>
...
function DiscoveryTileGrid({
  router,
  entries,
  textPrimary,
}: {
  router: any;
  entries: HomeTileEntry[];
  textPrimary: string;
}) {
  ...
  return (
    <View style={styles.discoveryGrid}>
      {entries.map(({ tile, category }, index) => {
        const title = t(category.homeTitleKey, {
          defaultValue: category.fallbackTitle,
        });
        const onPress = () =>
          router.push({
        ...
        if (tile.size === "icon") {
          return (
            ...
            >
              ...
              <Text
                style={[styles.iconTileLabel, { color: textPrimary }]}
                numberOfLines={2}
              >
                {title}
              </Text>
        ...
        return (
          ...
          <AnimatedPressable
            ...
            onPress={onPress}
            style={
              tile.size === "long" ? styles.longTileCard : styles.discoveryCard
            }
            entering={FadeInDown.delay(index * 60)
              .duration(360)
              .springify()}
...
function DiscoveryTileGridSkeleton({
  isDark,
  tiles,
}: {
  isDark: boolean;
 ...
 */
function computeEditorLayout(tiles: HomeCategoryTile[]): {
  rects: Map<DiscoveryCategoryId, TileRect>;
  height: number;
} {
  ...
  for (const tile of tiles) {
    ...
    const h =
      EDITOR_FACE_HEIGHT[tile.size] +
      (tile.size === "icon" ? EDITOR_ICON_LABEL_HEIGHT : 0);
...
function EditorTile({
  ...
  const resizePan = Gesture.Pan()
    ...
    .onUpdate((event) => {
      ...
      const maxW =
        EDITOR_TILE_SIZE_STEPS[EDITOR_TILE_SIZE_STEPS.length - 1].width +
        RESIZE_OVERDRAG;
      const nextW = Math.min(
        maxW,
        Math.max(minW, resizeStartW.value + event.translationX),
      );
      pw.value = nextW;
      let nearest = EDITOR_TILE_SIZE_STEPS[0];
      for (const step of EDITOR_TILE_SIZE_STEPS) {
        if (Math.abs(step.width - nextW) < Math.abs(nearest.width - nextW))
          nearest = step;
      }
  ...
  const nextSize =
    EDITOR_TILE_SIZE_STEPS[
      (EDITOR_TILE_SIZE_STEPS.findIndex((step) => step.size === tile.size) +
        1) %
        EDITOR_TILE_SIZE_STEPS.length
    ].size;
  const nextSizeLabel =
    nextSize === "icon" ? "compact" : nextSize === "card" ? "card" : "wide";
    ...
    <Animated.View style={tileStyle} exiting={FadeOut.duration(140)}>
      ...
      <GestureDetector gesture={tileGesture}>
        <Animated.View style={[styles.editorFace, faceStyle]}>
          ...
          <Animated.View
            ...
            style={[
              styles.editorFaceClip,
              tile.size === "icon" && styles.editorFaceClipIcon,
            ]}
          >
            <DiscoveryTileFace item={category} size={tile.size} title={title} />
          </Animated.View>
      ...
      {canRemove && (
        <Pressable
          ...
          accessibilityLabel={t("home.discoveryEditor.remove", {
      ...
      <GestureDetector gesture={resizePan}>
        <Pressable
          ...
          accessibilityRole="button"
          accessibilityLabel={t("home.discoveryEditor.resize", {
            defaultValue: "Resize {{title}} to {{size}}",
            title,
            size: nextSizeLabel,
          })}
...
function HomeCategoriesEditor({
  ...
  const [draggingId, setDraggingId] = useState<DiscoveryCategoryId | null>(
    null,
  );
  const [resizingId, setResizingId] = useState<DiscoveryCategoryId | null>(
    null,
  );
  ...
  const handleDragMove = useCallback(
    (id: DiscoveryCategoryId, dx: number, dy: number) => {
  ...
  const handleResize = useCallback(
    (id: DiscoveryCategoryId, size: DiscoveryTileSize) => {
      void haptics.selection();
      setDraft((prev) =>
        prev.map((entry) => (entry.id === id ? { ...entry, size } : entry)),
      );
    },
    [],
  );
          ...
          >
            ...
            <View style={styles.editorHeader}>
              <View style={{ flex: 1 }}>
                <Text style={[styles.editorTitle, { color: textPrimary }]}>
                  {t("home.discoveryEditor.title", {
                    defaultValue: "Customize categories",
                  })}
                </Text>
                <Text style={[styles.editorSubtitle, { color: textSecondary }]}>
                  {t("home.discoveryEditor.subtitle", {
                    defaultValue:
                      "Hold and drag to arrange. Drag a tile’s edge to resize, − to remove.",
                  })}
                </Text>
              </View>
              <TouchableOpacity
            ...
            >
                ...
                {draft.map((entry) => {
                  ...
                  return (
                    <EditorTile
                      ...
                      gestureEnabled={
                        activeId === null || activeId === entry.id
                      }
              ...
              {available.length > 0 && (
                <>
                  <Text
                    style={[styles.editorMoreTitle, { color: textSecondary }]}
                  >
                    {t("home.discoveryEditor.more", {
                  ...
                  <View style={styles.editorAddGrid}>
                    {available.map((category) => (
                      ...
                      >
                        ...
                        <View
                          style={[styles.editorBadge, styles.editorAddBadge]}
                        >
...
function OpportunityCard({
  ...
  const locationLabel = item.isRemote
    ? t("opportunityCard.remote")
    : (item.location ?? "").trim();
  ...
  return (
    ...
    >
        ...
        <View>
          ...
          {category ? (
            <View style={styles.oppCategoryChip}>
              <Text style={styles.oppCategoryChipText} numberOfLines={1}>
                {category}
              </Text>
            </View>
          ) : null}
      ...
      <View style={styles.oppCardContent}>
        <View style={styles.oppCardTop}>
          {showMatch ? (
            ...
            >
              ...
              <Text style={[styles.oppMatchBadgeText, { color: accent }]}>
                {t("opportunityCard." + MATCH_TIER_KEY[getMatchTier(matchPct)])}
              </Text>
            </View>
          ) : showOrg ? (
            <View
              style={[
                styles.oppOrgBadge,
                {
                  backgroundColor: isDark ? "rgba(99,102,241,0.15)" : "#F0F0FF",
                },
              ]}
            >
              <Text style={styles.oppOrgText} numberOfLines={1}>
                {orgLabel}
              </Text>
            </View>
          ) : (
          ...
          <View style={styles.oppCardActions}>
            ...
            {onBookmark && (
              ...
              >
                <BookmarkPlus
                  size={16}
                  color={bookmarked ? "#6366F1" : textSecondary}
                  fill={bookmarked ? "#6366F1" : "transparent"}
                />
              </TouchableOpacity>
            )}
          </View>
        ...
        <Text
          style={[styles.oppTitle, { color: textPrimary }]}
          numberOfLines={2}
        >
          {item.title}
        </Text>
        {showOrgLine ? (
          <Text
            style={[styles.oppOrgLine, { color: textSecondary }]}
            numberOfLines={1}
          >
            {orgLabel}
          </Text>
        ...
        {showMatchReason && (
          <Text style={styles.oppMatchReason} numberOfLines={1}>
            {topMatchReason}
          </Text>
        )}
          ...
          <View style={styles.oppMetaRow}>
            {locationLabel ? (
              <View style={styles.oppLocationRow}>
                ...
                <Text
                  style={[styles.oppLocationText, { color: textSecondary }]}
                  numberOfLines={1}
                >
                  {locationLabel}
                </Text>
            ...
            {!hasImage && category ? (
              ...
              >
                <Text
                  style={[styles.oppCategoryPillText, { color: textSecondary }]}
                  numberOfLines={1}
                >
                  {category}
                </Text>
        ...
        >
          <View style={styles.deadlineRow}>
            <View
              style={[styles.deadlineDot, { backgroundColor: deadlineColor }]}
            />
      ...
      <View style={[styles.posterTopRow, hero && styles.posterTopRowHero]}>
        {category ? (
          <View style={styles.posterCategoryChip}>
            <Text
              style={styles.posterCategoryText}
              numberOfLines={1}
        ...
        <View style={styles.posterTopActions}>
          ...
          {onBookmark && (
            ...
            >
              <BookmarkPlus
                size={13}
                color={bookmarked ? "#A5B4FC" : "#FFFFFF"}
                fill={bookmarked ? "#A5B4FC" : "transparent"}
              />
            </TouchableOpacity>
          )}
        </View>
      ...
      <View style={[styles.posterBottom, hero && styles.posterBottomHero]}>
        {matchPct > 0 && (
          <View style={styles.posterMatchBadge}>
            ...
            <Text style={styles.posterMatchText} maxFontSizeMultiplier={1.2}>
              {matchPct}% match
            </Text>
          </View>
        )}
        <Text
          style={hero ? styles.posterTitleHero : styles.posterTitle}
          numberOfLines={2}
          maxFontSizeMultiplier={1.2}
        >
          {item.title}
        ...
        <View style={styles.posterFooterRow}>
          <View style={styles.deadlineRow}>
            ...
            />
            <Text
              style={[styles.posterDeadline, { color: deadlineColor }]}
              maxFontSizeMultiplier={1.2}
            >
    ...
    >
      {imageUri ? (
        <ImageBackground
          ...
          imageStyle={
            hero ? styles.posterImageRadiusHero : styles.posterImageRadius
          }
          resizeMode="cover"
          onError={() => setImageFailed(true)}
        >
...
function FeaturedCarousel({
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      listRef.current?.scrollToOffset({
        offset: pageRef.current * FEATURED_PAGE_SNAP,
        animated: true,
      });
  ...
  const handleMomentumEnd = (e: NativeSyntheticEvent<NativeScrollEvent>) => {
    const page = Math.max(
      ...
      Math.min(
        pageCount - 1,
        Math.round(e.nativeEvent.contentOffset.x / FEATURED_PAGE_SNAP),
      ),
    );
    ...
    <View>
      <FlatList
        ...
        getItemLayout={(_, index) => ({
          length: FEATURED_ITEM_SNAP,
          offset: FEATURED_ITEM_SNAP * index,
          index,
        })}
...
function _BestShotCard({
  ...
  return (
    ...
    >
      ...
      <View
        style={[
          styles.bestShotContent,
          hasImage && styles.bestShotContentPoster,
        ]}
      >
        <View style={styles.bestShotTopRow}>
          ...
          <View
            style={[
              styles.deadlineRow,
              hasImage && styles.bestShotDeadlineChip,
            ]}
          >
            <View
              style={[
                styles.deadlineDot,
                { backgroundColor: deadlineTextColor },
              ]}
            />
            <Text
              style={[styles.bestShotDeadline, { color: deadlineTextColor }]}
            >
              {deadlineBadge.shortLabel}
            </Text>
          </View>
        ...
        <View>
          {artHasTitle ? null : (
            <Text
              style={[styles.bestShotTitle, { color: titleColor }]}
              numberOfLines={2}
            >
              {item.title}
            </Text>
          ...
          {topReason ? (
            <Text
              style={[styles.bestShotReason, { color: reasonColor }]}
              numberOfLines={2}
            >
              {topReason}
            </Text>
          ...
          <View style={{ flex: 1 }}>
            ...
            >
              {profileComplete
                ...
                : "Complete your profile for sharper matches"}
            </Text>
          </View>
        </View>
        {items.length > 0 ? (
  ...
  return (
    ...
    >
      ...
      >
        ...
        <LinearGradient
          ...
          colors={
            hasImage
              ...
              : isDark
                ? ["#17366F", "#102A59", "#151D45"]
                : ["#E7F1FF", "#EEF0FF", "#FFFFFF"]
          }
          locations={[0, 0.58, 1]}
          start={{ x: 0, y: 0.5 }}
          end={{ x: 1, y: 0.5 }}
        ...
        <Text
          style={[
            styles.nextMoveEyebrow,
            { color: hasImage || isDark ? "#7DD3FC" : "#2563EB" },
          ]}
        >
        ...
        <Text
          style={[
            styles.nextMoveTitle,
            { color: hasImage || isDark ? "#F8FAFC" : "#0F172A" },
          ]}
          numberOfLines={2}
        >
          {model.title}
        ...
        <Text
          style={[
            styles.nextMoveSupporting,
            { color: hasImage || isDark ? "#FBBF24" : "#B45309" },
          ]}
          numberOfLines={1}
        >
          {model.supporting}
...
export default function Dashboard() {
  ...
  const opportunityPipelineHomeEnabled = useFeatureFlag(
  ...
  const [recentlyOpenedOpportunity, setRecentlyOpenedOpportunity] =
    useState<RecentlyOpenedOpportunity | null>(null);
  const [opportunityRotationSeed, setOpportunityRotationSeed] = useState(() =>
    createOpportunityRotationSeed(),
  );
  ...
  useEffect(() => {
    const fetchBookmarks = async () => {
      ...
      try {
        ...
        const uniqueBookmarkIds = Array.from(
          new Set(bookmarks?.map((b) => b.opportunity_id) || []),
        );
  ...
  const syncOpportunityWidget = useCallback(
    async (freshOpportunities: Opportunity[]) => {
  ...
  const { completeness: profileCompleteness, isLoading: profileLoading } =
  ...
  const personalizedBestMatches = useMemo(() => {
    const eligibleCandidates = profileReady
      ...
      : rankedMatchCandidates.filter(
          (item) => bestShotScore(item) >= BEST_SHOT_MIN_MATCH,
        );
  ...
  const bestForYouIds = useMemo(
    () => new Set(bestForYou.map((o) => o.id)),
    [bestForYou],
  );
  ...
  useEffect(() => {
    let cancelled = false;
    const unsubscribe = subscribeToRecentlyOpenedOpportunity(
      userId,
      (opportunity) => {
  ...
  const nextMove = useMemo<HomeFocusModel | null>(() => {
    ...
    return {
      ...
      image:
        recentlyOpenedOpportunity.image ?? currentOpportunity?.image ?? null,
    };
  ...
  const recordOpportunityOpen = useCallback(
        ...
        <Animated.View entering={FadeInDown.duration(400).delay(50)}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity
              ...
              accessibilityLabel={t("home.discoveryEditor.title", {
                defaultValue: "Customize categories",
              })}
            >
            ...
            <View style={styles.sectionHeader}>
              <View style={styles.sectionTitleGroup}>
                ...
                <Text
                  ...
                  maxFontSizeMultiplier={1.3}
                >
                  {t("home.recommendedOpportunities", {
              ...
              <AnimatedPressable
                ...
                hapticFeedback="light"
                scaleTo={0.9}
                accessibilityLabel={t("home.viewMore", {
