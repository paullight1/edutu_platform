...
export default function IntentionalOpportunityHomeSection({
  ...
  if (loading && !data) {
    return (
      <View style={[styles.loadingCard, { backgroundColor: colors.card }]}>
        ...
        <Text style={[styles.loadingText, { color: colors.textSecondary }]}>Preparing your opportunity path…</Text>
      </View>
    );
  }
  ...
  if (!data) {
    return (
      ...
      >
        <Text style={[styles.errorTitle, { color: colors.foreground }]}>Your path could not load</Text>
        <Text style={[styles.errorText, { color: colors.textSecondary }]}>{error ?? 'Check your connection and try again.'}</Text>
        <Pressable
  ...
  return (
    <View style={styles.section} accessibilityLabel="Your intentional opportunity path">
      {isStale ? (
        <Text style={[styles.stale, { color: colors.mutedForeground }]}>Showing your last synced path</Text>
      ) : null}
      <CurrentFocusCard intent={data.intent} onEdit={() => setEditingFocus(true)} />
      ...
      <View style={styles.recommendations}>
        <View style={styles.sectionHeader}>
          <View style={styles.sectionHeaderCopy}>
            <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Recommended for your focus</Text>
            <Text style={[styles.sectionSubtitle, { color: colors.textSecondary }]}>Three intentional choices, not an endless feed.</Text>
          </View>
          <Pressable
            accessibilityRole="button"
        ...
        {data.degraded ? (
          <Text style={[styles.degraded, { color: colors.warning }]}>Personalisation is temporarily limited; your active path is still available.</Text>
        ) : null}
        ...
        {recommendations.length === 0 ? (
          <Text style={[styles.empty, { color: colors.textSecondary }]}>You have decided on this shortlist. Explore more opportunities below.</Text>
        ) : null}
      </View>
      <CurrentFocusSheet
...
const styles = StyleSheet.create({
  ...
  loadingCard: { marginHorizontal: 16, marginBottom: 20, minHeight: 112, borderRadius: 18, alignItems: 'center', justifyContent: 'center', gap: 10 },
  loadingText: { fontSize: 13, fontWeight: '600' },
  errorCard: { marginHorizontal: 16, marginBottom: 20, borderRadius: 18, borderWidth: 1, padding: 16, gap: 8 },
  errorTitle: { fontSize: 17, fontWeight: '800' },
  errorText: { fontSize: 13, lineHeight: 18 },
  retry: { alignSelf: 'flex-start', minHeight: 42, borderWidth: 1, borderRadius: 11, flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 13 },
