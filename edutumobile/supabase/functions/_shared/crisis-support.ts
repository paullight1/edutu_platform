/**
 * ============================================================================
 *  HAND-KEPT DENO MIRROR — DO NOT let this drift.
 * ============================================================================
 * This is a self-contained Deno port of the pure crisis-detection logic in
 *
 *     backend/services/services/api/src/chat/crisis-support.ts
 *
 * That Nest module is the SOURCE OF TRUTH. This file exists only because the
 * chat-proxy edge function is the fallback used precisely when the Nest backend
 * is unreachable, so it cannot import from it. Any change to the detection
 * patterns, the supported-locale set, the localized support messages, the
 * `{{contact}}` slot, or the findahelpline.com line in the Nest module MUST be
 * mirrored here by hand — nothing checks this file (it is Deno, excluded from
 * tsconfig/eslint, and there is no `deno` binary in CI). A missed mirror means a
 * user in crisis gets caught and answered in the Nest path but NOT in this
 * fallback. Treat the two files as one.
 * ============================================================================
 *
 ...
 * safety path can render. The Nest backend makes this admin-configurable, but
 * this edge function is the fallback used exactly when Nest may be unreachable,
 * so it CANNOT read admin settings — a crisis response must never depend on a
 * network read. The baked-in default is the correct fail-safe here.
 */
export const DEFAULT_CRISIS_CONTACT = "+2348169400427";

/**
 * Placeholder replaced with the crisis contact number. findahelpline.com is a
 * fixed professional directory that resolves to staffed crisis lines by
 * country — it stays verbatim.
 */
 ...
 * optional contact number degrades to the baked-in DEFAULT_CRISIS_CONTACT so the
 * safety message always renders even without an admin-configured value.
 */
