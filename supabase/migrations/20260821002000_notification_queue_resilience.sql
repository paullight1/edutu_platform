...
create or replace function private.notification_queue_retry_delay(
...
begin
  if tg_op = 'INSERT' then
    -- Normal queued broadcasts write an in-app row first. Give every one a
    -- stable dedupe key so a database/worker retry cannot create a second
    -- inbox row or duplicate interruption. Push-only quiet-hours redeliveries
    -- deliberately skip this because their original inbox row already exists.
  ...
  -- Existing application code records a failed delivery as status='failed'.
  -- Convert that transition atomically into either a bounded retry or a
  -- terminal dead letter so callers cannot forget retry policy.
  if new.status = 'failed' and old.status = 'processing' then
    ...
    v_error := left(
      coalesce(new.result ->> 'error', 'Notification queue delivery failed'),
      1000
    );
...
-- Historical terminal failures predate retry-safe dedupe semantics. Preserve
-- them as dead letters for operator inspection rather than risking duplicate
-- push/email delivery by replaying them automatically.
...
begin
  with stale as (
    ...
    set attempt_count = stale.attempt_count + 1,
        ...
        locked_at = null,
        status = case
          when stale.retry_safe
            and stale.attempt_count + 1 < stale.max_attempts
          then 'pending'
          else 'dead_letter'
        end,
        scheduled_for = case
...
-- Queue state is an internal control-plane resource. RLS was already enabled,
-- but browser roles did not need table privileges at all. Removing the grants
-- gives defense in depth if a future policy is accidentally broadened.
