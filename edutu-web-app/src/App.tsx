...
import { parseEdutuDeepLink } from "./lib/deepLinks";
    ...
    <PublicEditorialShell>
      <div className="flex min-h-[calc(100dvh-180px)] items-center justify-center px-6 py-12">
        <div className="w-full max-w-md rounded-[28px] border border-slate-200 bg-white p-6 text-left shadow-sm dark:border-white/10 dark:bg-slate-950">
          <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-300"><ShieldAlert className="h-6 w-6" aria-hidden="true" /></div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-950 dark:text-white">Admin access denied</h1>
          <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">{message}</p>
          <div className="mt-7 grid gap-3 sm:grid-cols-2">
            <button type="button" onClick={() => navigate(-1)} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-white/10 dark:text-slate-200 dark:hover:bg-white/5"><ArrowLeft className="h-4 w-4" aria-hidden="true" />Go back</button>
            <button type="button" onClick={() => navigate("/dashboard", { replace: true })} className="rounded-2xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200">Open dashboard</button>
          </div>
        </div>
      </div>
    </PublicEditorialShell>
...
function AdminPortalGate() {
  ...
  useEffect(() => {
    ...
    async function verifyAccess() {
      ...
      if (!nextUrl) {
        setGate({ status: "denied", message: "The admin portal URL is not configured safely. Contact the Edutu team before trying again." });
        return;
      }
      try {
        ...
        if (!isActive) return;
        setGate({ status: "denied", message: "This account is signed in, but it does not have permission to open Edutu admin." });
      }
    }
    void verifyAccess();
  ...
  return <PublicEditorialShell><div className="flex min-h-[calc(100dvh-180px)] items-center justify-center px-6 text-center"><div className="max-w-sm"><div className="mx-auto mb-4 h-12 w-12 animate-spin rounded-full border-4 border-brand-500/25 border-t-brand-500" /><p className="text-sm font-medium text-slate-600 dark:text-slate-300">{gate.status === "allowed" ? "Opening the admin portal" : gate.message || "Checking admin access"}</p></div></div></PublicEditorialShell>;
}
...
function DocsRedirect() {
  if (!isExternalDocsUrl()) return <Navigate to={DOCS_SITE_URL} replace />;
  return <ExternalDocsRedirect />;
}

function ExternalDocsRedirect() {
  useEffect(() => { window.location.replace(DOCS_SITE_URL); }, []);
  return <PublicEditorialShell><div className="flex min-h-[calc(100dvh-180px)] items-center justify-center px-6 text-center"><div className="max-w-sm"><div className="mx-auto mb-4 h-12 w-12 animate-spin rounded-full border-4 border-brand-500/25 border-t-brand-500" /><p className="text-sm font-medium text-slate-600 dark:text-slate-300">Opening Scholarship Engine docs</p></div></div></PublicEditorialShell>;
}

function MarketingRedirect({ path, fallback }: { path: string; fallback: ReactNode }) {
  useEffect(() => {
    if (!MARKETING_SITE_URL) return;
    try { window.location.replace(new URL(path, MARKETING_SITE_URL).toString()); } catch { /* render fallback */ }
  }, [path]);
  if (!MARKETING_SITE_URL) return <>{fallback}</>;
  return <PublicEditorialShell><div className="flex min-h-[calc(100dvh-180px)] items-center justify-center px-6 text-center"><div className="max-w-sm"><div className="mx-auto mb-4 h-12 w-12 animate-spin rounded-full border-4 border-brand-500/25 border-t-brand-500" /><p className="text-sm font-medium text-slate-600 dark:text-slate-300">Opening Edutu</p></div></div></PublicEditorialShell>;
}

function UserDashboardPage() {
  const navigate = useNavigate();
  const { user } = useAppAuth();
  const openOpportunity = useCallback((opportunity: { id?: string }) => {
    if (!opportunity?.id) return;
    navigate(`/app/opportunity/${opportunity.id}`, { state: { opportunity } });
  }, [navigate]);
  const openAppRoute = useCallback((screen: string) => {
    if (screen === "opportunities" || screen.startsWith("opportunities?")) { navigate(`/app/${screen}`); return; }
    if (screen === "deadlines" || screen === "saved" || screen === "applied" || screen === "profile") {
      if (screen === "applied") { navigate("/app/applications"); return; }
      navigate(`/app/${screen}`); return;
    }
    if (screen === "home") navigate("/dashboard");
  }, [navigate]);
  return <Dashboard user={user} onOpportunityClick={openOpportunity} onViewAllOpportunities={() => navigate("/app/opportunities")} onNavigate={openAppRoute} embeddedDesktopShell />;
}

function AppWorkspaceRoute({ children }: { children: ReactNode }) {
  return <ProtectedRoute><AppWorkspaceShell>{children}</AppWorkspaceShell></ProtectedRoute>;
}
  ...
  return (
    <>
      ...
      <Suspense fallback={<PageSuspense />}>
        <Routes>
          <Route path="/" element={<LandingPageV3 onGetStarted={handleGetStarted} />} />
          <Route path="/dashboard" element={<AppWorkspaceRoute><UserDashboardPage /></AppWorkspaceRoute>} />
          <Route path="/app/home" element={<AppWorkspaceRoute><UserDashboardPage /></AppWorkspaceRoute>} />
          <Route path="/opportunities" element={<OpportunitiesPage />} />
          <Route path="/app/opportunities" element={<AppWorkspaceRoute><OpportunitiesPage embedded /></AppWorkspaceRoute>} />
          <Route path="/app/submit-opportunity" element={<AppWorkspaceRoute><SubmitOpportunityPage /></AppWorkspaceRoute>} />
          ...
          <Route path="/edutuforyou/stories/:slug" element={<EdutuForYouStoryPage />} />
          <Route path="/upgrade" element={<UpgradePage />} />
          <Route path="/pro" element={<Navigate to="/upgrade" replace />} />
          <Route path="/what-we-believe" element={<WhatWeBelievePage />} />
          <Route path="/beliefs" element={<Navigate to="/what-we-believe" replace />} />
          <Route path="/our-belief" element={<Navigate to="/what-we-believe" replace />} />
          <Route path="/community" element={<CommunityPage />} />
          <Route path="/app/community" element={<AppWorkspaceRoute><CommunityWorkspacePage /></AppWorkspaceRoute>} />
          ...
          <Route path="/scholarship-engine" element={<MarketingRedirect path="/scholarship-api" fallback={<ScholarshipApiPage />} />} />
          <Route path="/scholarship-api" element={<MarketingRedirect path="/scholarship-api" fallback={<Navigate to="/scholarship-engine" replace />} />} />
          <Route path="/developers" element={<MarketingRedirect path="/scholarship-api" fallback={<DevelopersLandingPage />} />} />
          <Route path="/developers/docs" element={<DeveloperDocsPage />} />
          <Route path="/dashboard/developer" element={<ProtectedRoute><DeveloperDashboardPage /></ProtectedRoute>} />
          ...
          <Route path="/roadmaps" element={<AppWorkspaceRoute><RoadmapsPage /></AppWorkspaceRoute>} />
          <Route path="/roadmaps/:id" element={<Navigate to="/roadmaps" replace />} />
          <Route path="/app/roadmaps" element={<AppWorkspaceRoute><RoadmapsPage /></AppWorkspaceRoute>} />
          ...
          <Route path="/saved" element={<AppWorkspaceRoute><SavedPage /></AppWorkspaceRoute>} />
          <Route path="/app/saved" element={<AppWorkspaceRoute><SavedPage /></AppWorkspaceRoute>} />
          <Route path="/applications" element={<AppWorkspaceRoute><ApplicationsPage /></AppWorkspaceRoute>} />
          <Route path="/applied" element={<Navigate to="/applications" replace />} />
          <Route path="/app/applications" element={<AppWorkspaceRoute><ApplicationsPage /></AppWorkspaceRoute>} />
          <Route path="/app/applied" element={<Navigate to="/app/applications" replace />} />
          <Route path="/notifications" element={<AppWorkspaceRoute><NotificationsPage /></AppWorkspaceRoute>} />
          <Route path="/app/notifications" element={<AppWorkspaceRoute><NotificationsPage /></AppWorkspaceRoute>} />
          <Route path="/profile" element={<AppWorkspaceRoute><ProfilePage /></AppWorkspaceRoute>} />
          <Route path="/app/profile" element={<AppWorkspaceRoute><ProfilePage /></AppWorkspaceRoute>} />
          <Route path="/app/personalization" element={<ProtectedRoute><PersonalizationScreen /></ProtectedRoute>} />
          <Route path="/settings" element={<AppWorkspaceRoute><SettingsPage /></AppWorkspaceRoute>} />
          <Route path="/app/settings" element={<AppWorkspaceRoute><SettingsPage /></AppWorkspaceRoute>} />
          <Route path="/deadlines" element={<AppWorkspaceRoute><DeadlinesPage /></AppWorkspaceRoute>} />
          <Route path="/app/deadlines" element={<AppWorkspaceRoute><DeadlinesPage /></AppWorkspaceRoute>} />
          <Route path="/goals" element={<AppWorkspaceRoute><GoalsPage /></AppWorkspaceRoute>} />
          <Route path="/app/goals" element={<AppWorkspaceRoute><GoalsPage /></AppWorkspaceRoute>} />
          ...
          <Route path="/opportunity/:id" element={<OpportunityDetailFetcher onBack={() => navigate("/opportunities")} />} />
          <Route path="/app/opportunity/:id" element={<AppWorkspaceRoute><OpportunityDetailFetcher onBack={() => navigate("/app/opportunities")} embedded /></AppWorkspaceRoute>} />
