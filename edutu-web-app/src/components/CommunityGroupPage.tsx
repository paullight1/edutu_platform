            ...
            <div key={member.membership.id} className="flex flex-col gap-3 py-4 first:pt-0 last:pb-0 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                {member.membership.role === "owner" ? (
                  <span className="inline-flex h-10 items-center rounded-xl bg-brand-500/10 px-3 text-xs font-semibold text-brand-700">Owner</span>
                ) : currentAdminIsOwner ? (
                  <select value={member.membership.role} disabled={busyId === member.membership.id} onChange={(event) => void changeRole(member, event.target.value as MemberRole)} aria-label={`Role for ${member.profile.displayName}`} className="h-10 rounded-xl border border-subtle bg-surface-body px-3 text-xs font-semibold"><option value="member">Member</option><option value="mod">Moderator</option></select>
                ) : (
            ...
            {!contentLoading && tab === "chat" ? (
              <section className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
                <div className="overflow-hidden rounded-[28px] border border-subtle bg-surface-layer shadow-sm">
                  ...
                  <form onSubmit={handleSend} className="border-t border-subtle bg-surface-layer p-3 sm:p-4"><div className="flex items-end gap-2"><input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp,application/pdf" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleFile(file); }} /><button type="button" disabled={uploading || Boolean(group.archivedAt)} onClick={() => fileInputRef.current?.click()} aria-label="Attach image or PDF" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-subtle text-text-secondary transition hover:bg-surface-elevated disabled:opacity-50">{uploading ? <Loader2 size={18} className="animate-spin" /> : <Paperclip size={18} />}</button><label className="min-w-0 flex-1"><span className="sr-only">Message {group.name}</span><textarea name="message" aria-label={`Message ${group.name}`} defaultValue="" maxLength={4000} onInput={(event) => setComposerHasText(Boolean(event.currentTarget.value.trim()))} rows={1} disabled={Boolean(group.archivedAt)} placeholder={group.archivedAt ? "This group is archived" : "Write a message"} className="max-h-36 min-h-11 w-full resize-none rounded-2xl border border-subtle bg-surface-body px-4 py-3 text-sm leading-5 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 disabled:opacity-60" /></label><button type="submit" aria-label="Send message" disabled={sending || !composerHasText || Boolean(group.archivedAt)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-brand-500 text-white transition hover:bg-brand-600 disabled:opacity-50">{sending ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />}</button></div></form>
                </div>
