    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/50 p-0 backdrop-blur-sm sm:items-center sm:p-6">
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 id="community-report-title" className="text-xl font-semibold">
              Report {target.label}
            </h2>
            <p className="mt-2 text-sm leading-6 text-text-secondary">
              Tell Edutu what is wrong. Reports are reviewed without exposing
              your identity to group owners.
            </p>
          </div>
          <button
            type="button"
            ...
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl hover:bg-surface-elevated"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <button
            type="button"
            onClick={onClose}
            className="min-h-11 rounded-2xl border border-subtle px-5 text-sm font-semibold"
          >
            Cancel
          </button>
          <button
            ...
            className="min-h-11 rounded-2xl bg-danger px-5 text-sm font-semibold text-white disabled:opacity-50"
          >
...
function JoinGate({
  ...
  if (status === "pending") {
    return (
      <div className="rounded-[28px] border border-warning/20 bg-warning/10 p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-text-secondary">
          You will be able to read and post in this group only after an owner or
          moderator approves you.
        </p>
      </div>
    );
  }
  ...
  if (status === "banned" || status === "removed") {
    return (
      <div className="rounded-[28px] border border-danger/20 bg-danger/10 p-6">
        <h2 className="text-lg font-semibold">
          This group is not available to your account
        </h2>
        <p className="mt-2 text-sm leading-6 text-text-secondary">
          You cannot rejoin from the browser. Contact Edutu support if you
  ...
  if (status === "invited") {
    return (
      <div className="rounded-[28px] border border-brand-500/20 bg-brand-500/10 p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-text-secondary">
          Accepting gives you access to this group’s messages, resources, and
          member list.
        </p>
        <button
          type="button"
  ...
  if (group.visibility === "private") {
    return (
      <div className="rounded-[28px] border border-subtle bg-surface-layer p-6">
        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-surface-elevated text-text-secondary">
          <Lock size={19} />
        </div>
        <h2 className="mt-4 text-lg font-semibold">Invite-only group</h2>
        <p className="mt-2 text-sm leading-6 text-text-secondary">
          Only members with a valid invitation can enter this private room.
        </p>
      </div>
    );
  }
  ...
  if (group.joinPolicy === "open") {
    return (
      <div className="rounded-[28px] border border-brand-500/20 bg-brand-500/5 p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-text-secondary">
          This is an open group. Join to read the full discussion, share
          resources, and meet members.
        </p>
        <button
          type="button"
          disabled={busy}
          onClick={() => onJoin([])}
          className="mt-5 min-h-11 rounded-2xl bg-brand-500 px-5 text-sm font-semibold text-white disabled:opacity-60"
        >
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm leading-6 text-text-secondary">
        The group owner reviews requests before private discussion becomes
        visible.
      </p>
      {questionsLoading ? (
        <div className="mt-5 flex items-center gap-2 text-sm text-text-muted">
          <Loader2 size={17} className="animate-spin" /> Loading questions…
        </div>
      ) : (
        <div className="mt-5 space-y-4">
          ...
          {!questions.length ? (
            <p className="text-sm text-text-muted">
              No questions are required. Send your request when ready.
            </p>
          ) : null}
        </div>
      )}
      <button
        type="submit"
        disabled={busy || questionsLoading}
        className="mt-5 min-h-11 rounded-2xl bg-brand-500 px-5 text-sm font-semibold text-white disabled:opacity-60"
      >
...
function AdminPanel({
  ...
  const currentAdminIsOwner = detail.membership?.role === "owner";
  ...
  const decide = async (
    ...
    try {
      ...
      onNotice({
        tone: "success",
        text:
          decision === "approved" ? "Member approved." : "Request declined.",
      });
    } catch (cause) {
      onNotice({
        tone: "error",
  ...
  const changeRole = async (
    ...
    try {
      ...
      onMembersChange(
        members.map((row) =>
          row.membership.id === member.membership.id
  ...
  const remove = async (member: CommunityMemberSummary) => {
    ...
    try {
      ...
      onMembersChange(
        members.filter((row) => row.membership.id !== member.membership.id),
      );
  ...
  const saveSettings = async (event: React.FormEvent) => {
    ...
    try {
      ...
      onNotice({
        ...
        text: isCommunityApiError(cause)
          ? cause.message
          : "Group settings could not be saved.",
      });
    } finally {
      setBusyId(null);
    }
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      >
        <h2 className="flex items-center gap-2 text-lg font-semibold">
          <Settings2 size={19} className="text-brand-500" /> Group settings
        </h2>
        <label className="mt-5 block text-sm font-semibold">
          ...
          <input
            ...
            maxLength={80}
            required
            className="mt-2 h-12 w-full rounded-2xl border border-subtle bg-surface-body px-4 font-normal outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20"
          />
        </label>
        <label className="mt-4 block text-sm font-semibold">
          ...
          <textarea
            ...
            maxLength={500}
            rows={4}
            className="mt-2 w-full resize-none rounded-2xl border border-subtle bg-surface-body px-4 py-3 font-normal outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20"
          />
        </label>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="text-sm font-semibold">
            ...
            <select
              ...
              className="mt-2 h-12 w-full rounded-2xl border border-subtle bg-surface-body px-4 font-normal"
            >
          ...
          <label className="text-sm font-semibold">
            ...
            <select
              ...
              onChange={(event) =>
                setJoinPolicy(event.target.value as "open" | "request")
              }
              className="mt-2 h-12 w-full rounded-2xl border border-subtle bg-surface-body px-4 font-normal disabled:opacity-60"
            >
      ...
      <div className="space-y-5">
        ...
        >
          <h2 className="flex items-center gap-2 text-lg font-semibold">
            <UserPlus size={19} className="text-brand-500" /> Invite member
          </h2>
          <p className="mt-2 text-sm leading-6 text-text-secondary">
            Invite using the member’s Edutu user ID. The backend validates
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row">
            <input
              ...
              className="h-12 min-w-0 flex-1 rounded-2xl border border-subtle bg-surface-body px-4 text-sm outline-none focus:border-brand-500"
              placeholder="Edutu user ID"
            />
            <button
              type="submit"
              disabled={busyId === "invite" || !inviteeId.trim()}
              className="min-h-12 rounded-2xl bg-brand-500 px-5 text-sm font-semibold text-white disabled:opacity-50"
            >
              {busyId === "invite" ? "Sending…" : "Invite"}
            </button>
          </div>
        </form>
        ...
        {currentAdminIsOwner ? (
          <div className="rounded-[28px] border border-danger/20 bg-danger/5 p-5 sm:p-6">
            <h2 className="flex items-center gap-2 text-lg font-semibold">
              <Archive size={19} className="text-danger" /> Archive group
            </h2>
            <p className="mt-2 text-sm leading-6 text-text-secondary">
              Archiving stops new activity and removes the group from active
              discovery without deleting its audit history.
            </p>
            <button
              type="button"
              disabled={busyId === "archive"}
              onClick={() => void archive()}
              className="mt-4 min-h-11 rounded-2xl border border-danger/30 px-5 text-sm font-semibold text-danger disabled:opacity-60"
            >
              {busyId === "archive" ? "Archiving…" : "Archive group"}
            </button>
          </div>
        ) : null}
      </div>

      <CommunityGroupSettingsTools
        detail={detail}
              ...
              >
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="min-h-10 rounded-xl border border-subtle px-4 text-xs font-semibold"
                  >
                    Decline
                  </button>
                  <button
                    ...
                    className="min-h-10 rounded-xl bg-brand-500 px-4 text-xs font-semibold text-white"
                  >
                    Approve
                  </button>
                </div>
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {member.membership.role === "owner" ? (
                    <span className="inline-flex h-10 items-center rounded-xl bg-brand-500/10 px-3 text-xs font-semibold text-brand-700">
                      Owner
                    </span>
                  ) : currentAdminIsOwner ? (
                    <select
                      value={member.membership.role}
                      disabled={busyId === member.membership.id}
                      onChange={(event) =>
                        void changeRole(
                          member,
                      ...
                      className="h-10 rounded-xl border border-subtle bg-surface-body px-3 text-xs font-semibold"
                    >
                      <option value="member">Member</option>
                      <option value="mod">Moderator</option>
                    </select>
                  ) : (
                    <span className="inline-flex h-10 items-center rounded-xl bg-surface-elevated px-3 text-xs font-semibold text-text-secondary">
                  ...
                  (currentAdminIsOwner ||
                    ...
                    <button
                      ...
                      className="inline-flex h-10 items-center gap-1.5 rounded-xl border border-danger/20 px-3 text-xs font-semibold text-danger"
                    >
...
export default function CommunityGroupPage() {
  ...
  const [reportTargetState, setReportTargetState] =
    ...
    try {
      ...
      setError(
        isCommunityApiError(cause)
          ? cause.message
          : "This Community group could not be loaded.",
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (
      ...
      detail.group.joinPolicy !== "request" ||
      detail.group.visibility !== "public"
    )
      return;
    let cancelled = false;
      ...
      try {
        ...
        if (showLoader) {
          ...
          setResources((current) =>
            mergeResources(current, resourcePage.resources),
          );
        }
        setMembers(
          memberPage.members.filter(
  ...
  const handleJoin = async (answers: JoinRequestAnswer[]) => {
    ...
    try {
      ...
      setNotice({
        ...
        text: isCommunityApiError(cause)
          ? cause.message
          : "Your membership could not be updated.",
      });
    } finally {
      setJoinBusy(false);
    }
  ...
  const handleSend = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const body = typeof formValue === "string" ? formValue.trim() : "";
    if (!body) return;
    setSending(true);
    setNotice(null);
    try {
  ...
  const handleFile = async (file: File) => {
    ...
    if (
      (!isImage && !isPdf) ||
      (isImage && file.size > COMMUNITY_IMAGE_MAX_BYTES) ||
      (isPdf && file.size > COMMUNITY_PDF_MAX_BYTES)
    ) {
      setNotice({
        tone: "error",
        text: "Choose a JPEG, PNG, or WebP image up to 5 MB, or a PDF up to 10 MB.",
      });
      return;
    }
    ...
    try {
      const reservation = await createCommunityAttachmentUpload(
        detail.group.id,
        {
          kind,
          name: file.name,
          mime: file.type as CommunityAttachment["mime"],
          size: file.size,
        },
        getToken,
      );
      ...
      setMessages((current) => mergeMessages(current, [message]));
      const resourcePage = await fetchGroupResources(
        detail.group.id,
        { limit: GROUP_PAGE_SIZE },
        getToken,
      );
      ...
      setNotice({
        ...
        text: isCommunityApiError(cause)
          ...
          : cause instanceof Error
            ? cause.message
            : "The resource could not be shared.",
      });
    } finally {
      setUploading(false);
  ...
  const submitReport = async (reason: string) => {
    ...
    try {
      await reportTarget(
        {
          targetType: reportTargetState.type,
          targetId: reportTargetState.id,
          reason,
        },
        getToken,
      );
  ...
  const handleBlock = async (member: CommunityMemberSummary) => {
    try {
      ...
      setMessages((current) =>
        current.filter((row) => row.userId !== member.membership.userId),
      );
      setNotice({
        tone: "success",
        text: `${member.profile.displayName} is blocked.`,
      });
    } catch (cause) {
      setNotice({
        tone: "error",
  ...
  if (loading) {
    return (
      <main className="min-h-[70dvh] bg-surface-body px-4 py-8">
        <div className="mx-auto max-w-7xl animate-pulse space-y-4">
          <div className="h-48 rounded-[32px] bg-surface-elevated" />
          <div className="h-96 rounded-[28px] bg-surface-elevated" />
        </div>
      </main>
    );
  }
  ...
  if (error || !detail) {
    return (
      <main className="min-h-[70dvh] bg-surface-body px-4 py-8">
        <div className="mx-auto max-w-2xl rounded-[28px] border border-danger/20 bg-surface-layer p-7">
          <h1 className="text-2xl font-semibold">Group unavailable</h1>
          ...
          <div className="mt-5 flex gap-2">
            <Link
              to="/app/community"
              className="min-h-11 rounded-2xl border border-subtle px-5 py-3 text-sm font-semibold"
            >
              Back to Community
            </Link>
            <button
              ...
              className="min-h-11 rounded-2xl bg-brand-500 px-5 text-sm font-semibold text-white"
            >
              Try again
            </button>
          </div>
  ...
  return (
    <main className="min-h-[calc(100dvh-4rem)] bg-surface-body px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
      <div className="mx-auto max-w-7xl">
        <Link
          to="/app/community"
          className="mb-4 inline-flex min-h-10 items-center gap-2 rounded-xl px-2 text-sm font-semibold text-text-secondary transition hover:bg-surface-elevated hover:text-text-primary"
        >
        ...
        <section className="relative overflow-hidden rounded-[32px] border border-subtle bg-surface-layer shadow-sm">
          <div className="relative min-h-[190px] overflow-hidden bg-gradient-to-br from-brand-600 via-brand-500 to-brand-800 p-6 text-white sm:p-8">
            ...
            <div className="relative flex h-full flex-col justify-between gap-8">
              <div className="flex items-start justify-between gap-4">
                <span className="flex h-14 w-14 items-center justify-center rounded-[20px] border border-white/20 bg-white/15 text-3xl backdrop-blur">
                  {group.coverEmoji || "💬"}
                </span>
                <div className="flex gap-2">
                  {group.visibility === "private" ? (
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-950/50 px-3 py-1.5 text-xs font-semibold">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setReportTargetState({
                        type: "group",
                        id: group.id,
                        label: "group",
                      })
                    }
                    className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-950/45 text-white transition hover:bg-slate-950/70"
                  >
                    <Flag size={15} />
                  </button>
                </div>
              ...
              <div className="max-w-3xl">
                ...
                <p className="mt-3 max-w-2xl text-sm leading-6 text-white/85 sm:text-base">
                  {group.description ||
                    "A focused Edutu group for learning, progress, and useful peer support."}
                </p>
                <div className="mt-4 flex flex-wrap gap-4 text-xs font-semibold text-white/80">
                  ...
                  <span className="inline-flex items-center gap-1.5">
                    <MessageCircle size={14} /> {group.messageCount} posts
                  </span>
                  {detail.membership?.status === "active" ? (
                    <span className="inline-flex items-center gap-1.5">
                      <Shield size={14} />{" "}
        ...
        {!isActive ? (
          <div className="mt-5 max-w-3xl">
            <JoinGate
              ...
              busy={joinBusy}
              onJoin={(answers) => void handleJoin(answers)}
            />
          </div>
        ) : (
          <>
            <div className="mt-5 border-b border-subtle">
              <div
                role="tablist"
              ...
              >
                {tabs.map(({ value, label, icon: Icon }) => (
                  <button
                    ...
                    className={`relative flex min-h-12 shrink-0 items-center gap-2 px-4 text-sm font-semibold transition ${tab === value ? "text-brand-700" : "text-text-muted hover:text-text-primary"}`}
                  >
                    <Icon size={17} /> {label}
                    <span
                      className={`absolute inset-x-3 bottom-0 h-0.5 rounded-full bg-brand-500 ${tab === value ? "opacity-100" : "opacity-0"}`}
                    />
                  </button>
                ))}
              </div>
            ...
            {contentLoading ? (
              <div className="mt-6 flex min-h-60 items-center justify-center rounded-[28px] border border-subtle bg-surface-layer">
                <Loader2
                  size={24}
                  className="animate-spin text-brand-500"
                  aria-label="Loading group activity"
                />
              </div>
            ) : null}
              ...
              <section className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
                <div className="overflow-hidden rounded-[28px] border border-subtle bg-surface-layer shadow-sm">
                  <div className="border-b border-subtle px-4 py-4 sm:px-5">
                    ...
                    <p className="mt-1 text-xs text-text-muted">
                      Keep conversation useful, respectful, and relevant to the
                      group. New activity refreshes while this tab is visible.
                    </p>
                  </div>
                  <div
                    className="max-h-[58dvh] min-h-[340px] overflow-y-auto px-4 py-5 sm:px-5"
                    aria-live="polite"
                  >
                    {messages.length ? (
                      <div className="space-y-5">
                        {messagesHasMore ? (
                          <div className="flex justify-center">
                            <button
                              ...
                              className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-subtle bg-surface-body px-4 text-xs font-semibold text-text-secondary transition hover:bg-surface-elevated disabled:opacity-60"
                            >
                        ...
                        {messages.map((message) => {
                          ...
                          return (
                            ...
                            >
                              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-surface-elevated text-xs font-semibold">
                                {initials(
                              ...
                              <div
                                className={`min-w-0 max-w-[82%] ${mine ? "text-right" : ""}`}
                              >
                                <div
                                  className={`flex flex-wrap items-center gap-2 ${mine ? "justify-end" : ""}`}
                                >
                                ...
                                <div
                                  className={`mt-1 rounded-[20px] px-4 py-3 text-left text-sm leading-6 ${mine ? "bg-brand-500 text-white" : "bg-surface-elevated text-text-primary"}`}
                                >
                                  {deleted ? (
                                    ...
                                    <div>
                                      ...
                                      <button
                                        ...
                                        className={`mt-3 inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold ${mine ? "bg-white/15 text-white" : "bg-surface-layer text-brand-700"}`}
                                      >
                                ...
                                {!deleted ? (
                                  <div
                                    className={`mt-1 flex gap-2 ${mine ? "justify-end" : ""}`}
                                  >
                                    {mine ? (
                                      <button
                                        type="button"
                                        ...
                                        className="inline-flex items-center gap-1 text-2xs font-semibold text-text-muted hover:text-danger"
                                      >
                      ...
                      <div className="flex min-h-[300px] flex-col items-center justify-center text-center">
                        ...
                        <p className="mt-2 max-w-sm text-sm leading-6 text-text-secondary">
                          Share progress, ask a focused question, or add a
                          resource that helps the group move forward.
                        </p>
                      </div>
                    )}
                  </div>
                  <form
                    ...
                    data-keyboard-scope
                    onSubmit={handleSend}
                    className="border-t border-subtle bg-surface-layer p-3 sm:p-4"
                  >
                    <div className="flex items-end gap-2">
                      ...
                      <button
                        type="button"
                        disabled={uploading || Boolean(group.archivedAt)}
                        onClick={() => fileInputRef.current?.click()}
                        aria-label="Attach image or PDF"
                        className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-subtle text-text-secondary transition hover:bg-surface-elevated disabled:opacity-50"
                      >
                        {uploading ? (
                      ...
                      <label className="min-w-0 flex-1">
                        <span className="sr-only">Message {group.name}</span>
                        <textarea
                          name="message"
                          ...
                          className="max-h-36 min-h-11 w-full resize-none rounded-2xl border border-subtle bg-surface-body px-4 py-3 text-sm leading-5 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 disabled:opacity-60"
                        />
                      </label>
                      <button
                        type="submit"
                        ...
                        className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-brand-500 text-white transition hover:bg-brand-600 disabled:opacity-50"
                      >
                        {sending ? (
                          <Loader2 size={18} className="animate-spin" />
                        ) : (
                ...
                <aside className="space-y-4">
                  <div className="rounded-[24px] border border-subtle bg-surface-layer p-5">
                    <h2 className="text-sm font-semibold">Room guide</h2>
                    <div className="mt-3 space-y-3 text-sm leading-6 text-text-secondary">
                      <p>Keep personal information private.</p>
                      <p>
                        Share resources you trust and explain why they are
                        useful.
                      </p>
                      <p>
                        Use report and block controls when something feels
                        unsafe.
                      </p>
                    </div>
                  </div>
                  <div className="rounded-[24px] border border-brand-500/15 bg-brand-500/5 p-5">
              ...
              <section className="mt-6">
                <div className="mb-4">
                  ...
                  <p className="mt-1 text-sm text-text-secondary">
                    Images and PDFs shared by active group members.
                  </p>
                </div>
                {resources.length ? (
                  <>
                    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                      {resources.map((resource) => (
                        <article
                          key={resource.id}
                          className="rounded-[24px] border border-subtle bg-surface-layer p-5 shadow-sm"
                        >
                          ...
                          <p className="mt-1 text-xs text-text-muted">
                            Shared by {resource.sender.displayName} ·{" "}
                            {formatDate(resource.createdAt)}
                          </p>
                          <button
                            type="button"
                            onClick={() =>
                              void openAttachment(resource.attachment.url)
                            }
                            className="mt-4 inline-flex min-h-10 items-center gap-2 rounded-xl border border-subtle px-4 text-xs font-semibold"
                          >
                            <Download size={14} /> Open resource
                          </button>
                        </article>
                    ...
                    {resourceCursor ? (
                      <div className="mt-5 flex justify-center">
                        <button
                          type="button"
                          disabled={loadingOlderResources}
                          onClick={() => void loadOlderResources()}
                  ...
                  <div className="rounded-[28px] border border-dashed border-subtle bg-surface-layer p-10 text-center">
                    <FileText size={26} className="mx-auto text-brand-500" />
                    <h3 className="mt-4 font-semibold">No resources yet</h3>
                    <p className="mt-2 text-sm text-text-secondary">
                      Share a useful image or PDF from the Chat tab.
                    </p>
                  </div>
                )}
              </section>
              ...
              <section className="mt-6">
                <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-text-secondary">
                      People with active access to this room.
                    </p>
                  </div>
                  {detail.membership?.role !== "owner" ? (
                    <button
                      ...
                      className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-danger/20 px-4 text-sm font-semibold text-danger disabled:opacity-60"
                    >
                      <LogOut size={16} />{" "}
            ...
            {!contentLoading && tab === "admin" && isAdmin ? (
              <section className="mt-6">
                <AdminPanel
                  detail={detail}
                  members={members}
      ...
      <ReportDialog
        ...
        busy={reportBusy}
        onClose={() => setReportTargetState(null)}
        onSubmit={(reason) => void submitReport(reason)}
      />
      <CommunityActionSheet
        open={leaveConfirmOpen}
        title="Leave community"
        description={`Leave ${group.name}? You may need to request access again later.`}
        confirmLabel="Leave"
        busy={leaveBusy}
