...
export default function ScraperDashboard() {
    ...
    const API_URL = `${backendBaseUrl}/api/scraper`;
    ...
    async function loadData() {
        ...
        try {
            ...
            let statsData: Array<{ source?: string | null }> = [];
            try {
    ...
    async function startScrape(sourceId?: number) {
        ...
        if (sourceId && sourcesToScrape.length === 0) {
            setScrapeResult({
                success: false,
                error: 'Selected source is disabled. Enable it first.',
            });
            return;
        }
        ...
        try {
            ...
            const progressSources = sourcesToScrape.length > 0
                ? sourcesToScrape
                : [{ name: 'Default Edutu sources', url: '', id: 0 } as ScrapeSource];
            setScrapingProgress(progressSources.map(s => ({ source: s.name, status: 'scraping' as const, progress: 0 })));
            ...
            if (response.ok) {
                ...
                const mapped: ScrapeResult = {
                    ...
                    sourcesScraped: result.sourcesScraped ?? progressSources.length,
                    totalResults: result.totalResults ?? 0,
                    duration: result.duration,
                    error: result.error,
                    sourceResults: result.sourceResults ?? progressSources.map(s => ({
                        name: s.name,
                        url: s.url,
                ...
                setScrapingProgress(
                    (result.sourceResults ?? progressSources.map(s => ({ name: s.name, status: 'success' }))).map((sr: any) => ({
                        source: sr.name,
