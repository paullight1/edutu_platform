...
export default function Opportunities() {
    ...
    const getAddMethodTheme = (id: string) => {
        const themes: Record<string, { bg: string; hover: string; icon: string; text: string; border: string }> = {
            manual: {
                bg: 'linear-gradient(135deg, rgba(59, 130, 246, 0.18), rgba(37, 99, 235, 0.08))',
                hover: 'linear-gradient(135deg, rgba(59, 130, 246, 0.28), rgba(37, 99, 235, 0.14))',
                icon: 'linear-gradient(135deg, #3b82f6, #1d4ed8)',
                text: '#93c5fd',
                border: 'rgba(59, 130, 246, 0.28)'
            },
            url: {
                bg: 'linear-gradient(135deg, rgba(14, 165, 233, 0.2), rgba(6, 182, 212, 0.08))',
                hover: 'linear-gradient(135deg, rgba(14, 165, 233, 0.32), rgba(6, 182, 212, 0.16))',
                icon: 'linear-gradient(135deg, #0ea5e9, #06b6d4)',
                text: '#7dd3fc',
                border: 'rgba(14, 165, 233, 0.3)'
            },
            'apify-all': {
                bg: 'linear-gradient(135deg, rgba(99, 102, 241, 0.2), rgba(79, 70, 229, 0.08))',
                hover: 'linear-gradient(135deg, rgba(99, 102, 241, 0.32), rgba(79, 70, 229, 0.16))',
                icon: 'linear-gradient(135deg, #6366f1, #4f46e5)',
                text: '#c4b5fd',
                border: 'rgba(99, 102, 241, 0.3)'
            },
            'apify-intel': {
                bg: 'linear-gradient(135deg, rgba(139, 92, 246, 0.2), rgba(124, 58, 237, 0.08))',
                hover: 'linear-gradient(135deg, rgba(139, 92, 246, 0.32), rgba(124, 58, 237, 0.16))',
                icon: 'linear-gradient(135deg, #8b5cf6, #7c3aed)',
                text: '#ddd6fe',
                border: 'rgba(139, 92, 246, 0.3)'
            },
            'apify-custom': {
                bg: 'linear-gradient(135deg, rgba(16, 185, 129, 0.18), rgba(5, 150, 105, 0.08))',
                hover: 'linear-gradient(135deg, rgba(16, 185, 129, 0.3), rgba(5, 150, 105, 0.16))',
                icon: 'linear-gradient(135deg, #10b981, #059669)',
                text: '#a7f3d0',
                border: 'rgba(16, 185, 129, 0.28)'
            },
            'apify-edutu': {
                bg: 'linear-gradient(135deg, rgba(255, 102, 0, 0.22), rgba(255, 69, 0, 0.09))',
                hover: 'linear-gradient(135deg, rgba(255, 102, 0, 0.34), rgba(255, 69, 0, 0.18))',
                icon: 'linear-gradient(135deg, #ff6600, #ff4500)',
                text: '#fed7aa',
                border: 'rgba(255, 102, 0, 0.3)'
            },
            'apify-scholarship-api': {
                bg: 'linear-gradient(135deg, rgba(168, 85, 247, 0.2), rgba(59, 130, 246, 0.08))',
                hover: 'linear-gradient(135deg, rgba(168, 85, 247, 0.32), rgba(59, 130, 246, 0.16))',
                icon: 'linear-gradient(135deg, #a855f7, #3b82f6)',
                text: '#e9d5ff',
                border: 'rgba(168, 85, 247, 0.3)'
            }
        };

        return themes[id] ?? themes.manual;
    };
    ...
    async function triggerApifySync(sourceId: string) {
        ...
        try {
            setLoadingStatus({ message: `Starting ${sourceId} scrape...`, progress: 30, source: sourceId, phase: 'preview' });
            ...
            if (response.ok && result.success) {
                ...
                setLoadedResults(opportunities);
                setLoadingStatus({
                    message: `Scrape finished: ${result.totalResults ?? opportunities.length ?? 0} opportunities processed`,
                    progress: 100,
                    source: sourceId,
                    phase: 'preview'
                });
    ...
    async function fetchOpportunities() {
        ...
        try {
            ...
            console.error('Failed to load opportunities from API, falling back to Supabase:', error);
    ...
    async function loadOpportunitiesFromSupabase() {
        ...
        const opps = ((data || []) as Opportunity[]).map((opp) => ({
            ...opp,
            status: (opp.status || 'active') as OpportunityStatus,
            summary: opp.summary || '',
            description: opp.description || '',
            category: opp.category || 'Scholarships',
            organization: opp.organization || '',
            location: opp.location || '',
            application_url: opp.application_url || (opp as any).apply_url || (opp as any).external_url || '',
            close_date: opp.close_date || (opp as any).deadline || '',
            image_url: opp.image_url || '',
            views: opp.views || 0,
            applications: opp.applications || 0,
        }));

        const total = count || opps.length;
        setOpportunities(opps);
        setFilteredOpps(opps);
        ...
        setStats({
            total,
            active: opps.filter((opp) => opp.status === 'active').length,
            featured: opps.filter((opp) => opp.is_featured).length,
            expiringSoon: opps.filter((opp) => {
                if (!opp.close_date) return false;
                const diff = new Date(opp.close_date).getTime() - Date.now();
                return diff > 0 && diff <= 7 * 24 * 60 * 60 * 1000;
            }).length,
            needsReview: opps.filter((opp) => opp.status === 'pending_review' || opp.metadata?.needs_review).length,
        });
    }
