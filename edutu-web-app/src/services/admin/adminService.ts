...
import { isAdminRole, verifyAdminAccess } from "../../lib/adminAccess";
...
const adminPermissionMap: Record<AdminUser["role"], AdminPermission[]> = {
  super_admin: [
    "opportunities:manage",
    "opportunities:approve",
    "marketplace:moderate",
  ...
  moderator: ["marketplace:moderate", "notifications:send", "support:respond"],
  support_agent: ["support:respond"],
};

async function getVerifiedAdminAccess() {
  const {
    data: { session },
...
export async function checkAdminAccess(): Promise<{
  ...
  try {
    const access = await getVerifiedAdminAccess();
    if (!access) return { isAdmin: false, role: null };

    return {
      isAdmin: true,
      role: isAdminRole(access.role) ? access.role : null,
    };
  } catch (error) {
...
export async function getAdminPermissions(
  ...
  const access = await getVerifiedAdminAccess();
  if (!access || !isAdminRole(access.role)) return [];

  return adminPermissionMap[access.role];
}
