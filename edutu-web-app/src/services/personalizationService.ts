...
export async function completeOnboarding(
  ...
  const result = await saveUserPersonalization(userId, {
    ...data,
    experienceLevel:
      data.experienceLevel as UserPersonalization["experienceLevel"],
...
export async function generateRecommendationsForUser(
  ...
  const { data: opportunities, error: oppError } = await supabase
    ...
    .select("*")
    .or(
      `close_date.is.null,close_date.gte.${new Date().toISOString().split("T")[0]}`,
    );
  ...
  for (const opp of opportunities) {
    ...
    if (
      weights.category &&
      userProfile.preferredCategories.includes(opportunity.category)
    ) {
...
export async function getRecommendationsForUser(
  ...
  Array<{ opportunity: Opportunity; matchScore: number; matchReasons: any[] }>
> {
...
function getMatchReasons(
  ...
  const hasInterestMatch = userProfile.interests.some(
    (interest) =>
