const CALL_PATH =
  /^\/communities\/calls\/([0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\/?$/i;
...
export function isCommunityCallPath(path: string): boolean {
  try {
    const url = new URL(path, "https://app.edutu.invalid");
    return (
      url.origin === "https://app.edutu.invalid" && CALL_PATH.test(url.pathname)
    );
  } catch {
    return false;
  }
