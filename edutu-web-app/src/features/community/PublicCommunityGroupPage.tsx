...
import { ArrowLeft, ArrowRight, CalendarClock, MessageCircle, ShieldCheck, UsersRound } from "lucide-react";
...
export default function PublicCommunityGroupPage() {
  ...
  const jsonLd = useMemo(() => group ? [
    ...
    },
    {
      ...
      "@type": "BreadcrumbList",
      itemListElement: [
        { "@type": "ListItem", position: 1, name: "Community", item: toAbsoluteUrl("/community") },
        { "@type": "ListItem", position: 2, name: group.name, item: toAbsoluteUrl(canonicalPath) },
      ],
    },
  ] : undefined, [canonicalPath, description, group]);
      ...
      <main className="mx-auto w-full max-w-5xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        ...
        {loading ? (
          ...
          <section className="mt-6 rounded-[30px] border border-[#efd9ca] bg-white p-8 text-center shadow-sm dark:border-subtle dark:bg-surface-layer sm:p-12">
            <span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#fcead5] text-[#f45b16] dark:bg-brand/10 dark:text-brand"><ShieldCheck size={24} /></span>
            <h1 className="mt-4 font-display text-3xl font-semibold tracking-[-0.04em]">This community is not publicly available</h1>
            <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-[#765e55] dark:text-text-secondary">It may be private, archived, closed, or no longer exist. Edutu does not reveal which case applies to anonymous visitors.</p>
            <Link to="/community" className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-[#f45b16] px-4 text-sm font-bold text-white">Explore public communities</Link>
          </section>
        ) : (
          <>
            <article className="mt-6 overflow-hidden rounded-[32px] border border-[#efd9ca] bg-white shadow-[0_30px_80px_-55px_rgba(74,23,13,.6)] dark:border-subtle dark:bg-surface-layer">
              <div className="relative bg-[#4a170d] px-6 py-8 text-white sm:px-9 sm:py-10">
                <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_80%_15%,rgba(244,91,22,.32),transparent_30%)]" />
                <div className="relative flex flex-col gap-5 sm:flex-row sm:items-start">
                  <span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[20px] bg-white/10 text-3xl ring-1 ring-white/15">{group.coverEmoji}</span>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-orange-200">Public Edutu community</p>
                    <h1 className="mt-2 max-w-3xl font-display text-4xl font-semibold leading-[1.02] tracking-[-0.045em] text-white sm:text-5xl">{group.name}</h1>
                    <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm font-semibold text-white/75">
                      <span className="inline-flex items-center gap-1.5"><UsersRound size={16} /> {group.memberCount.toLocaleString()} members</span>
                      <span className="inline-flex items-center gap-1.5"><MessageCircle size={16} /> {group.messageCount.toLocaleString()} posts</span>
                      {group.expiresAt ? <span className="inline-flex items-center gap-1.5"><CalendarClock size={16} /> Active until {new Date(group.expiresAt).toLocaleDateString()}</span> : null}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-8 p-6 sm:p-9 lg:grid-cols-[minmax(0,1fr)_300px]">
                <div>
                  <h2 className="font-display text-2xl font-semibold tracking-[-0.03em] text-[#39180f] dark:text-text-primary">What this community is for</h2>
                  <p className="mt-4 whitespace-pre-wrap text-base leading-8 text-[#765e55] dark:text-text-secondary">{description}</p>
                  <div className="mt-7 rounded-[22px] border border-[#efd9ca] bg-[#fff9f1] p-4 dark:border-subtle dark:bg-surface-elevated">
                    <p className="text-sm font-bold text-[#4a170d] dark:text-text-primary">Public summary only</p>
                    <p className="mt-1 text-sm leading-6 text-[#765e55] dark:text-text-secondary">Messages, member identities, request answers, private resources and moderation data are never exposed on this public page. Sign in and follow the group’s membership rules to participate.</p>
                  </div>
                </div>
