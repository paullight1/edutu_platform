...
export default function CommunityGroupsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(isCommunityApiError(caught) ? caught.message : "We couldn't load your groups right now.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
      ...
      >
        {loading ? (
          ...
          <div className="space-y-8">
            {error ? (
              <div role="status" className="flex items-center justify-between gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-200">
                <span>{error}</span>
                <button
                  type="button"
                  onClick={() => {
                ...
                >
                  <RefreshCw size={15} className={refreshing ? "animate-spin" : ""} /> Retry
                </button>
              </div>
            ) : null}
            ...
            {invited.length > 0 ? (
              <GroupSection
                ...
                title="Invitations"
                body="An owner invited you. Open a group to preview it and decide whether to join."
                rows={invited}
              />
            ) : null}
...
function GroupSection({
  ...
  return (
    <section>
      <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#f45b16] dark:text-brand">{eyebrow}</p>
      <div className="mt-1 flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
        <div>
          <h2 className="font-display text-xl font-semibold tracking-[-0.02em] text-[#4a170d] dark:text-text-primary">{title}</h2>
          <p className="mt-1 max-w-2xl text-sm leading-6 text-[#796f6b] dark:text-text-secondary">{body}</p>
        </div>
        <span className="text-xs font-bold text-[#9a8278] dark:text-text-muted">{rows.length}</span>
      </div>
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {rows.map((row) => <GroupCard key={row.group.id} row={row} />)}
      </div>
    </section>
  );
}
