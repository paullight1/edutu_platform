...
export default function CommunityGroupSettingsPage() {
  ...
  const [confirmation, setConfirmation] = useState<
    | { kind: "remove"; member: CommunityMemberSummary }
    | { kind: "archive" }
    | null
  >(null);
  const canManage = detail ? canManageCommunityGroup(detail, userId) : false;
  ...
  const uploadCover = async (file: File | null) => {
    ...
    try {
      ...
      if (classification.kind !== "image") {
        throw new Error(
          "Community cover photos must be JPEG, PNG, or WebP images up to 5 MB.",
        );
      }
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "Cover photo could not be updated.",
      );
    } finally {
      setWorking(null);
    }
  ...
  const changeRole = async (
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Role could not be changed.",
      );
    } finally {
      setWorking(null);
    }
  ...
  const archive = async () => {
    ...
    try {
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "Community could not be archived.",
      );
    } finally {
      setWorking(null);
      setConfirmation(null);
        ...
        <div className="mx-auto max-w-4xl space-y-5">
          ...
          >
            <div className="grid gap-4 sm:grid-cols-[110px_minmax(0,1fr)]">
              <div>
                <p className="mb-2 text-xs font-bold text-[#6b4538] dark:text-text-secondary">
                  Cover
                </p>
              ...
              <div className="space-y-4">
                ...
                {group.opportunityId ? (
                  <p className="rounded-xl bg-[#fff9f1] px-3 py-2 text-xs leading-5 text-[#796f6b] dark:bg-surface-elevated dark:text-text-secondary">
                    This community is linked to opportunity{" "}
                    <strong>{group.opportunityId}</strong>. The link is fixed
                    after creation so history cannot silently move to a
              ...
              options={[
                {
                  value: "public",
                  label: "Public",
                  body: "Discoverable to signed-in members.",
                },
                {
                  value: "private",
                  label: "Private",
                  body: "Only active members and invitees can open it.",
                },
              ]}
            />
            <div className="mt-4">
                ...
                options={[
                  {
                    value: "open",
                    label: "Open",
                    body: "Visible users join immediately.",
                  },
                  {
                    value: "request",
                    label: "Approval required",
                    body: "Applicants wait for an owner/mod decision.",
                  },
                ]}
              />
            </div>
            ...
            action={
              joinPolicy === "request" && questions.length < 5 ? (
                <button
                  ...
                  className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-[#fcead5] px-3 text-xs font-bold text-[#d94b0f] dark:bg-brand/10 dark:text-brand"
                >
          ...
          >
            {joinPolicy !== "request" ? (
              <p className="text-sm leading-6 text-[#796f6b] dark:text-text-secondary">
                Questions are dormant while joining is open. Switching back to
                approval-required keeps the form available for editing.
              </p>
            ) : questions.length === 0 ? (
              <p className="text-sm leading-6 text-[#796f6b] dark:text-text-secondary">
                No screening questions. Applicants will still enter the pending
                queue for review.
              </p>
            ) : (
          ...
          <SettingsSection
            ...
            action={
              <Link
                to={`/app/community/groups/${id}/requests`}
                className="inline-flex min-h-10 items-center rounded-xl border border-[#f4dcc9] px-3 text-xs font-bold dark:border-subtle"
              >
                Review requests
              </Link>
            }
          >
            <div className="mb-4 flex gap-2">
              <input
                ...
                placeholder="Edutu user ID (user_…)"
                className="community-input min-w-0 flex-1"
              />
              <button
                type="button"
            ...
            <div className="divide-y divide-[#f4dcc9] dark:divide-subtle">
              {members.map((member) => {
                ...
                const canRemove =
                  !creator &&
                  !self &&
                  (isOwner || member.membership.role === "member");
                return (
                  <div
                    key={member.membership.id}
                    className="flex min-h-16 items-center gap-3 py-2"
                  >
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#fcead5] text-xs font-extrabold text-[#8f3f1b] dark:bg-surface-elevated">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-[#8d7b74] dark:text-text-muted">
                        {creator
                          ? "Creator · owner"
                    ...
                    {isOwner && !creator && !self ? (
                      <select
                        ...
                        disabled={
                          working === `role:${member.membership.userId}`
                        }
                        onChange={(event) =>
                          void changeRole(
                            member,
                        ...
                        className="h-10 rounded-xl border border-[#f4dcc9] bg-white px-2 text-xs font-bold dark:border-subtle dark:bg-surface-body"
                      >
                    ...
                    {canRemove ? (
                      <button
                        type="button"
                        disabled={
                          working === `remove:${member.membership.userId}`
                        }
                        onClick={() =>
                          setConfirmation({ kind: "remove", member })
                        }
                        aria-label={`Remove ${member.profile.displayName}`}
                        className="flex h-11 w-11 items-center justify-center rounded-xl text-[#a18c83] hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/10"
                      >
                        <UserMinus size={16} />
                      </button>
                    ) : null}
          ...
          >
            {blocks.length === 0 ? (
              <p className="text-sm text-[#796f6b] dark:text-text-secondary">
              ...
              <div className="divide-y divide-[#f4dcc9] dark:divide-subtle">
                {blocks.map((blocked) => (
                  ...
                  >
                    <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#fcead5] text-xs font-extrabold text-[#8f3f1b] dark:bg-surface-elevated">
                      {blocked.displayName.slice(0, 1).toUpperCase()}
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-bold">
                        {blocked.displayName}
                      </p>
                      <p className="truncate text-xs text-[#8d7b74] dark:text-text-muted">
                        {blocked.userId}
                      </p>
                    </div>
                    <button
                      type="button"
                      disabled={working === `unblock:${blocked.userId}`}
                      onClick={async () => {
                      ...
                      className="min-h-10 rounded-xl border border-[#f4dcc9] px-3 text-xs font-bold text-[#6b4538] dark:border-subtle dark:text-text-secondary"
                    >
                      Unblock
                    </button>
                  </div>
          ...
          <div
            data-keyboard-avoid
            className="sticky bottom-[calc(4.6rem+env(safe-area-inset-bottom))] z-20 flex items-center justify-between gap-3 rounded-[20px] border border-[#f4dcc9] bg-white/95 p-3 shadow-lg backdrop-blur dark:border-subtle dark:bg-surface-layer lg:bottom-4"
          >
            <p className="hidden text-xs leading-5 text-[#796f6b] dark:text-text-secondary sm:block">
              Changes to access and screening take effect after you save.
            </p>
            <button
              type="button"
              disabled={saving}
              onClick={() => void save()}
              className="ms-auto inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#f45b16] px-5 text-sm font-bold text-white disabled:opacity-60"
            >
              {saving ? (
                <Loader2 size={16} className="animate-spin" />
              ) : (
          ...
          {isOwner ? (
            <section className="rounded-[24px] border border-red-200 bg-red-50 p-5 dark:border-red-500/20 dark:bg-red-500/10">
              <div className="flex items-start gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-300">
                  <Archive size={18} />
                </span>
                <div className="min-w-0 flex-1">
                  <h2 className="text-sm font-extrabold text-red-900 dark:text-red-200">
                    Archive permanently
                  </h2>
                  <p className="mt-1 text-xs leading-5 text-red-700 dark:text-red-300">
                    Archiving is irreversible. The room becomes read-only and
              ...
              <button
                ...
                className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl bg-red-700 px-4 text-sm font-bold text-white disabled:opacity-60"
              >
                <Trash2 size={15} /> Archive community
              </button>
            </section>
          ) : null}
        </div>
      ...
      <CommunityActionSheet
        ...
        description={
          confirmation?.kind === "remove"
            ? `${confirmation.member.profile.displayName} will lose access to this community.`
            : "Archiving is permanent and makes this room read-only."
        }
        confirmLabel={confirmation?.kind === "archive" ? "Archive" : "Remove"}
        busy={working !== null}
        onClose={() => setConfirmation(null)}
        onConfirm={() => {
          if (confirmation?.kind === "remove")
            void removeMember(confirmation.member);
          else if (confirmation?.kind === "archive") void archive();
        }}
      />
    </>
  );
  ...
  return (
    <label className="block">
      <span className="mb-1.5 flex items-center justify-between gap-2 text-xs font-bold">
        <span>{label}</span>
        {hint ? (
          <span className="font-normal text-[#9a8278]">{hint}</span>
        ) : null}
      </span>
      {children}
    </label>
...
function ChoiceRow({
  ...
  options: Array<{ value: string; label: string; body: string }>;
}) {
  return (
    <fieldset>
      <legend className="mb-2 text-xs font-bold">{label}</legend>
      <div className="grid gap-2 sm:grid-cols-2">
        {options.map((option) => (
          <button
            ...
            className={`rounded-2xl border p-3 text-start ${value === option.value ? "border-[#f45b16] bg-[#fff4e9] dark:bg-brand/10" : "border-[#f4dcc9] bg-white dark:border-subtle dark:bg-surface-body"}`}
          >
            <span className="flex items-center gap-2 text-sm font-bold">
              {value === option.value ? (
                <Check size={15} className="text-[#f45b16]" />
              ) : null}
              {option.label}
            </span>
            <span className="mt-1 block text-xs leading-5 text-[#796f6b] dark:text-text-secondary">
              {option.body}
            </span>
          </button>
        ))}
...
function QuestionEditor({
  ...
  index: number;
  onChange: (question: GroupSettingsQuestionDraft) => void;
  onRemove: () => void;
}) {
  ...
  return (
    <div className="rounded-2xl border border-[#f4dcc9] bg-[#fffdf9] p-4 dark:border-subtle dark:bg-surface-body">
      <div className="flex items-start justify-between gap-3">
        <p className="text-xs font-extrabold uppercase tracking-[0.12em] text-[#9a8278]">
          Question {index + 1}
        </p>
        <button
          type="button"
          ...
          className="flex h-9 w-9 items-center justify-center rounded-xl text-[#a18c83] hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/10"
        >
          <X size={15} />
        </button>
      </div>
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        ...
        <Field label="Type">
          <select
            value={question.type}
            onChange={(event) =>
              setType(event.target.value as GroupSettingsQuestionDraft["type"])
            }
            className="community-input"
          >
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
        ...
        <label className="flex min-h-11 items-center gap-2 rounded-xl border border-[#f4dcc9] px-3 text-xs font-bold dark:border-subtle">
          <input
            type="checkbox"
      ...
      {question.type === "single_select" ? (
        <Field label="Options (one per line)" hint="2–6 options">
          <textarea
            ...
            className="community-input mt-3 resize-none"
          />
        </Field>
      ) : null}
    </div>
...
function Alert({
  ...
  return (
    <div
      ...
      className={`rounded-2xl border px-4 py-3 text-sm font-semibold ${success ? "border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-200" : "border-red-200 bg-red-50 text-red-700 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-200"}`}
    >
      {children}
    </div>
  );
